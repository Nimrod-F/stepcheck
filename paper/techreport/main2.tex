...
for that state kind---the raw result ($\top$) for \texttt{Task}/\texttt{Map}/\texttt{Parallel}, the
\texttt{Pass} result shape, or the effective input $\eff$ for \texttt{Choice}/\texttt{Wait}. In
...
closes the successor shape precisely, and a query \texttt{Task}/\texttt{Map}/\texttt{Parallel} with
no \texttt{Output} yields $\top$; \asl{}'s JSONata mode has no \texttt{InputPath}/\texttt{OutputPath}
