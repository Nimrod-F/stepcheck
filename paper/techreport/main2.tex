...
\texttt{ItemSelector}), in an \texttt{Assign}/\texttt{Arguments}/\texttt{Output} template or a
JSONata object literal (contributing variable names and produced field names), in the declared
input schema, or as a segment of a \texttt{ResultPath} or catch \texttt{ResultPath}. Both
components of the environment draw their keys from this same finite universe, so the argument
below bounds each; the environment lattice, a finite product of the shape lattice, then has
finite height whenever each component does. On the bounded-depth fragment there is a bound $D$ on the depth of
...
At the fixpoint---and only if the machine converged (Lemma~\ref{lem:term})---the analysis checks,
...
84/126 to 88/126; the paired oracle run confirms all 92 \code{SC1101} reports emitted under the
ablation with zero counterexamples.
