...
An implementation ablation reduces that opacity. The implementation also resolves quoted-bracket
member paths; with \texttt{--result-shapes}, \tool{} treats declared task-output schemas as
closed-world result records and adds closed top-level envelopes for common AWS integrations (Lambda
invoke, DynamoDB, SNS, SQS, EventBridge, and Step Functions child execution). Under this
service/output-shape modeling, \code{SC1101} recall improves from \textbf{84/126 to 88/126} (67\% to
70\%). The independent oracle
confirms all 92 \code{SC1101} reports emitted in the ablation run absent on all checked acyclic paths,
again with zero counterexamples; the remaining misses are behind unsupported or templated service
results, arrays, or complex selectors that still lift to $\top$.
