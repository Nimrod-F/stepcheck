...
bug behind opacity, but it cannot manufacture a definite absence. With a finite key universe and
bounded record depth, the lattice has finite height on every workflow topology. The implementation
uses $|\mathit{states}|\times(|\mathit{keys}|+2)\times(k+1)+2$ rounds as a safety bound and
telemetry denominator; if a bug ever exceeded it, all known entries would be lifted to $\top$ before
checking rather than reported from an under-approximation.

In practice the bound is not approached: across the 193 AWS, 66 CNCF, and two DSL workflows, all
790 data-flow fixpoints converge, with a maximum of 7 rounds and a worst observed bound utilization
of 2/54 (3.7\%).

Figure~\ref{fig:intro} marks the first ASL task, \texttt{Reserve Flight}, reading
\texttt{\$.flightTable} and \texttt{\$.outboundFlightId} through \texttt{Parameters}; in the upstream
template, those fields come from the AppSync resolver's execution payload. With a generated or
declared entry schema, omitting either field is a definite missing-field fault; with raw \asl{} input
alone, the initial document is $\top$ and the native pass stays silent. Loops use the same
abstraction: a state with a back-edge joins the pre-loop and loop-carried shapes, so \code{SC1101}
fires downstream only when \emph{no} iteration can produce the read field.
...
\paragraph{Proof.}
Let $\gamma$ map each abstract document/variable shape to the concrete JSON values it may denote,
with $\top$ denoting all values and records denoting exactly their listed keys. Each transfer
function over-approximates the corresponding \asl{} operation: constructors build closed records;
opaque tasks, arrays, unsupported selectors, and unsupported JSONata expressions lift to $\top$;
\texttt{ResultPath} and \texttt{Catch} place over-approximating shapes into the raw input; and
\texttt{InputPath}/\texttt{OutputPath} either navigate inside the current abstraction or return
$\top$ for unmodeled selection. Joins are least upper bounds, and depth-$k$ widening is extensive
because replacing a sub-record by $\top$ only enlarges $\gamma$. Therefore the forward iteration
computes a post-fixpoint that over-approximates every concrete environment reaching each state,
including loop iterations. If lookup of $p$ at the relevant read site reaches a missing key before
$\top$, then no concrete value in $\gamma$ contains that path. The modeled \asl{} read therefore
raises the runtime missing-reference error. Inputs and task results treated by declared schemas are
closed-world premises; expressions outside the modeled fragment reach $\top$ and cannot trigger the
premise of the theorem. \qed
...
[tasks."Reserve Booking"]
idempotent = false
persistent = true
compensation = "Cancel Booking"
state_in = "SeatReserved"
state_out = "BookingReserved"
\end{lstlisting}

The booking saga's complete sidecar is 69 lines of TOML---about eight per-task lines over four
tasks, plus shared schema and protocol blocks---and the typed DSL emits the same facts by
construction.
