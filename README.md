...
| Path | What |
|---|---|
| `paper/main.pdf` | the compiled paper (LNCS, **15 pp incl. references**, within the ICSOC limit). Sources in `paper/sections/`; the evaluation chart is generated by `eval/figures.ipynb` (matplotlib) → `paper/figures/eval-figure.pdf`; bibliography in `paper/references.bib` (every entry verified — see `paper/bib-verification.md`). |
...
| `eval/` | the evaluation harness and results: `SUMMARY.md`, `results.json` (E2/E3/E5), `inference_accuracy.json` (E4), `baseline-statelint.json` + `statelint_baseline.js` (E9 statelint baseline), `triage-verdicts.json` + `wf-eval.js` (warning-precision + gold-expansion), `stats.tex` (E1). |
...
- Mutation study: **429** injected defects, **100%** detection. Baseline `statelint` detects
  the structural (166/166) and broken-binding (140/141) classes but **0/103** unsafe retries
  and **0/19** missing compensations.
